...
function createHandler(config) {
  ...
  return async function handler(payload, context) {
    ...
    //    If the AI API is unreachable (all retries exhausted), degrade gracefully:
    //    post a "temporarily unavailable" comment and return a degraded result.
    ...
    try {
      ...
      context.log.error(
        `${name} - AI call failed: ${aiErr.message}. Posting degraded comment.`
      );

      const degradedComment =
        `<h3>${escapeHtml(name)} — AI Temporarily Unavailable</h3>\n` +
        `<p>The AI service could not be reached. This work item will be analysed once the service is restored.</p>\n` +
        "<hr/>\n" +
        "<em>Generated by DevOps AI Bot</em>";

      try {
        await postComment(data, degradedComment, context);
      } catch (postErr) {
        context.log.error(
          `${name} - Failed to post degraded comment: ${postErr.message}.`
        );
      }

      return buildResult(data, { error: aiErr.message, degraded: true });
    }
